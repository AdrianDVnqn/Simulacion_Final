# Simulación Final

Repositorio del trabajo práctico integral de Simulación.

## Flujo de trabajo recomendado

- `main` queda como rama estable.
- Cada integrante trabaja en una rama propia, por ejemplo `feat-introduccion` o `juan-desarrollo`.
- Antes de hacer cambios grandes, hacer `git pull` para partir de la última versión.
- Cuando la tarea está lista, subir la rama y abrir un Pull Request hacia `main`.
- No editar el mismo `.qmd` principal al mismo tiempo si se puede evitar.

## Recomendación para Quarto

- Mantener un archivo principal que arme el documento final.
- Separar el contenido por partes en archivos más chicos si el trabajo crece.
- Evitar subir PDFs durante el desarrollo.
- Compilar el PDF solo al final, cuando el texto esté cerrado.

## Archivos generados

Los archivos PDF quedan ignorados por Git para evitar conflictos con binarios generados localmente.
